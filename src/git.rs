//! Recognising Git repositories, worktrees, and submodules.
...
pub fn marks_repository(git_path: &Path) -> Result<bool> {
    match fs::entry_kind(git_path)? {
        Some(EntryKind::Directory) => Ok(true),
        Some(EntryKind::File) => links_to_repository(git_path),
        Some(EntryKind::Other) | None => Ok(false),
    }
}

fn links_to_repository(git_file: &Path) -> Result<bool> {
    // A redirection file is short text; binary contents cannot be one.
    let Some(contents) = fs::read_text(git_file)? else {
        return Ok(false);
    };
